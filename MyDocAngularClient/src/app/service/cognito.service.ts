...
export class CognitoUtil {
  ...
  getUserPool() {
      if (environment.cognito_idp_endpoint) {
          this._POOL_DATA.endpoint = environment.cognito_idp_endpoint;
      }
  ...
  buildCognitoCreds(idTokenJwt: string) {
      let url = 'cognito-idp.' + CognitoUtil._REGION.toLowerCase() + '.amazonaws.com/' + this._POOL_DATA.userPoolId;
      if (environment.cognito_idp_endpoint) {
          url = environment.cognito_idp_endpoint + '/' + this._POOL_DATA.userPoolId;
      }
      let logins: CognitoIdentity.LoginsMap = {};
      logins[url] = idTokenJwt;
      let params = {
          IdentityPoolId: this._POOL_DATA.identityPoolId, /* required */
          Logins: logins
      };
